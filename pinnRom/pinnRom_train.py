...
from pinnRom_model import PINN, Sampler
from base_rom import U_rom, U_fom
...
if __name__ == "__main__":
	...
	print('----------------------------------------')
	print(f"Burgers ROM closure with Re: {Re}")
	print('----------------------------------------')
	...
	# X_r, _ = res_sampler.sample((n_x, n_t))
	# x = X_r[:, 0:1]; t = X_r[:,1:2]
	# U_rom_vec = U_rom.flatten(order='F')[:,None]
	# x_grid = x.reshape(n_x, n_t)
	# t_grid = t.reshape(n_x, n_t)
	# # plt.plot(t_grid[:,0], U_rom[:,0])
	# # plt.show()
	...
	# # print(x[0:100])
	# # print(t[0:100])
	# # print(U_rom_vec[0:100])
	# sys.exit()

	rom_data = [U_rom, U_rom0, U_rom_WALL1, U_rom_WALL2]
	fom_data = U_fom
	...
	model = PINN(res_sampler, bcs_sampler, rom_data, fom_data, savept=save_folder + 'weights')
	# model = PINN(res_sampler, bcs_sampler, rom_data, savept=save_folder + 'weights')
	start = time.time()
	model.train()
	...
	u_pred = u_pred/SCALE + U_rom.transpose()
