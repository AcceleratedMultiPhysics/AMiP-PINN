...
class PINN_laminar_flow:
    ...
    def __init__(self, Collo, WALL_1, WALL_2, WALL_3, WALL_4, uvt_layers, lb, ub, ExistModel=0, uvtDir=''):
        ...
        self.u_WALL_3_pred, self.v_WALL_3_pred, self.p_WALL_3_pred, self.t_WALL_3_pred, self.ty_WALL_3_pred= self.net_uvt_neumann(self.x_WALL_3_tf, self.y_WALL_3_tf)
        self.u_WALL_4_pred, self.v_WALL_4_pred, self.p_WALL_4_pred, self.t_WALL_4_pred, self.ty_WALL_4_pred= self.net_uvt_neumann(self.x_WALL_4_tf, self.y_WALL_4_tf)
        ...
        self.loss_WALL_3 = tf.reduce_mean(tf.square(self.u_WALL_3_pred - self.u_WALL_3_tf)) \
                         ...
                         + tf.reduce_mean(tf.square(self.ty_WALL_3_pred))
        ...
        beta = 1.0
        self.loss = 2.0*self.loss_f + beta*(self.loss_WALL_1 + self.loss_WALL_2 + 2.0*self.loss_WALL_3 + 2.*self.loss_WALL_4)
    ...
    def net_f(self, x, y):
        ...
        f_v = rho*(u*v_x + v*v_y) + p_y - mu*(v_xx + v_yy) - t
...
def postProcess(xmin, xmax, ymin, ymax, field_MIXED, s=2, alpha=0.5, marker='o'):
    ...
    # plt.savefig('./uvpt_Rae5.png', dpi=300)
    plt.close('all')
...
if __name__ == "__main__":
    ...
    V1     = 0.0
    T1     = 1.0
    WALL_1  = [0.0, 0.0] + [0.0, 1.0] * lhs(2, Nwall1)
    ...
    V2     = 0.0
    T2     = 0.0
    WALL_2  = [1.0, 0.0] + [0.0, 1.0] * lhs(2, Nwall2)

    # WALL Lower (3) // Neumann Temperature, Dirichlet Velocity
    Nwall3 = 150 
    U3     = 0.0
    V3     = 0.0
    T3     = 0.0
    WALL_3  = [0.0, 0.0] + [1.0, 0.0] * lhs(2, Nwall3)

     # WALL Upper (4) // Neumann Temperature, Dirichlet Velocity
    Nwall4 = 150 
    U4     = 0.0
    V4     = 0.0
    T4     = 0.0
    WALL_4  = [0.0, 1.0] + [1.0, 0.0] * lhs(2, Nwall4)
    
    x_WALL_1 = WALL_1[:,0:1]
    y_WALL_1 = WALL_1[:,1:2]
    u_WALL_1 = U1*(1.0 + x_WALL_1)
    ...
    # plt.scatter(WALL_1[:, 1:2], WALL_1[:, 5:6], marker='o', alpha=0.2, color='red')
    # plt.show()

    x_WALL_2 = WALL_2[:,0:1]
    y_WALL_2 = WALL_2[:,1:2]
    u_WALL_2 = U2*(0.0 + x_WALL_2)
    v_WALL_2 = V2*(0.0 + x_WALL_2)
    p_WALL_2 = 0*(0.0 + x_WALL_2)
    t_WALL_2 = T2*(0.0 + x_WALL_2) 
    ...
    with tf.device('/device:GPU:0'):
        ...
        loss_WALL_1, loss_WALL_2, loss_WALL_3, loss_WALL_4, loss_f, loss = model.train(iter=Niter, learning_rate=5e-4)
        model.train_bfgs()
        print("--- %s seconds ---" % (time.time() - start_time))

        # Save neural network
        model.save_NN('cavity2D_Rae5_ratio2_long.pickle')


        # # Load trained neural network
        # model = PINN_laminar_flow(XY_c, WALL_1, WALL_2, WALL_3, WALL_4, uvt_layers, lb, ub, ExistModel = 1, uvtDir = 'cavity2D.pickle')

        # Save loss history
        with open('loss_history_Rae5_ratio2_long.pickle', 'wb') as f:
            pickle.dump(model.loss_rec, f)

        # Get mixed-form PINN prediction
        x_PINN = np.linspace(0, 1.0, 251)
        y_PINN = np.linspace(0, 1.0, 251)
        ...
        mdic = {"field_MIXED": field_MIXED, "label": "Ra1e5"}
        scipy.io.savemat("cavity2D_Rae5_ratio2_long.mat",mdict=mdic)


        # Plot the comparison of u, v, p, t
        postProcess(xmin=0, xmax=1.0, ymin=0, ymax=1.0, field_MIXED=field_MIXED, s=4, alpha=0.5)
