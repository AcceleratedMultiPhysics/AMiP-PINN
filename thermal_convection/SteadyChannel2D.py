...
class PINN_laminar_flow:
    ...
    def __init__(self, Collo, WALL_1, WALL_2, WALL_3, WALL_4, uvt_layers, lb, ub, ExistModel=0, uvtDir=''):
        ...
        # tf graphs
        self.u_pred, self.v_pred, self.p_pred, self.t_pred= self.net_uvt(self.x_tf, self.y_tf)
        
        self.f_pred_u, self.f_pred_v, self.f_pred_p, self.f_pred_t = self.net_f(self.x_c_tf, self.y_c_tf)
        self.u_WALL_1_pred, self.v_WALL_1_pred, self.p_WALL_1_pred, self.t_WALL_1_pred= self.net_uvt(self.x_WALL_1_tf, self.y_WALL_1_tf)
        self.u_WALL_2_pred, self.v_WALL_2_pred, self.p_WALL_2_pred, self.t_WALL_2_pred= self.net_uvt(self.x_WALL_2_tf, self.y_WALL_2_tf)
        ...
        self.u_WALL_4_pred, self.v_WALL_4_pred, self.p_WALL_4_pred, self.t_WALL_4_pred= self.net_uvt(self.x_WALL_4_tf, self.y_WALL_4_tf)
        ...
        self.loss_WALL_1 = tf.reduce_mean(tf.square(self.u_WALL_1_pred - self.u_WALL_1_tf)) \
                         ...
                         + tf.reduce_mean(tf.square(self.p_WALL_1_pred - self.p_WALL_1_tf))

        # Dirichlet Velocity and Dirichlet Temperature
        self.loss_WALL_2 = tf.reduce_mean(tf.square(self.u_WALL_2_pred - self.u_WALL_2_tf)) \
                         + tf.reduce_mean(tf.square(self.v_WALL_2_pred - self.v_WALL_2_tf)) \
                         + tf.reduce_mean(tf.square(self.t_WALL_2_pred - self.t_WALL_2_tf)) \
        ...
        self.loss_WALL_3 = tf.reduce_mean(tf.square(self.u_WALL_3_pred - self.u_WALL_3_tf)) \
                         ...
                         + tf.reduce_mean(tf.square(self.p_WALL_3_pred - self.p_WALL_3_tf))

        # Dirichlet Velocity and Dirichlet Temperature and Dirichlet Pressure     
        self.loss_WALL_4 = tf.reduce_mean(tf.square(self.u_WALL_4_pred - self.u_WALL_4_tf)) \
                         ...
                         + tf.reduce_mean(tf.square(self.t_WALL_4_pred - self.t_WALL_4_tf)) \
                         + tf.reduce_mean(tf.square(self.p_WALL_4_pred - self.p_WALL_4_tf))


        beta = 12.0
        self.loss = self.loss_f + beta*(self.loss_WALL_1 + self.loss_WALL_2 + self.loss_WALL_3 + self.loss_WALL_4)
    ...
    def net_f(self, x, y):
        ...
        f_v = rho*(u*v_x + v*v_y) + p_y - mu*(v_xx + v_yy) - Ra / (Pr * Re**2) * t
    ...
    def train(self, iter, learning_rate):
        ...
        return loss_WALL_1, loss_WALL_2, loss_WALL_3, loss_WALL_4, loss_f, self.loss
    ...
    def getloss(self):
        ...
        return loss_WALL_1, loss_WALL_2, loss_WALL_3, loss_WALL_4, loss
...
def postProcess(xmin, xmax, ymin, ymax, field_MIXED, s=2, alpha=0.5, marker='o'):
    ...
    cf = ax[1, 1].scatter(x_MIXED, y_MIXED, c=t_MIXED, alpha=alpha, edgecolors='none', cmap='rainbow', marker=marker, s=int(s))
    ax[1, 1].axis('square')
    ...
    ax[1, 1].set_title('Temperature (C)')
    fig.colorbar(cf, ax=ax[1, 1], fraction=0.046, pad=0.04)

    plt.savefig('uvpt_channel_Ra3_Dirichlet_p_w12.png', dpi=300)
    plt.close('all')
...
if __name__ == "__main__":
    ...
    V1     = 0.0
    T1     = 1.0
    WALL_1  = [0.0, -1.0] + [0.0, 2.0] * lhs(2, Nwall1)
    ...
    V2     = 0.0
    T2     = 1.0
    WALL_2  = [2.0, -1.0] + [0.0, 2.0] * lhs(2, Nwall2)
    ...
    V3     = 0.0
    T3     = 1.0
    WALL_3  = [0.0, -1.0] + [2.0, 0.0] * lhs(2, Nwall3)
    ...
    V4     = 0.0
    T4     = 0.0
    WALL_4  = [0.0, 1.0] + [2.0, 0.0] * lhs(2, Nwall4)
    ...
    x_WALL_1 = WALL_1[:,0:1]
    y_WALL_1 = WALL_1[:,1:2]
    u_WALL_1 = U1*(1.0 - np.square(y_WALL_1))
    v_WALL_1 = V1*(1.0 + x_WALL_1)
    p_WALL_1 = Ra / (2.0 * Pr * Re**2) * (y_WALL_1 - np.square(y_WALL_1)/2) - 2.0 * x_WALL_1 / Re
    # p_WALL_1 = 0*(1.0 + x_WALL_1)
    t_WALL_1 = T1*((1.0 - y_WALL_1)/2) 
    ...
    p_WALL_2 = Ra / (2.0 * Pr * Re**2) * (y_WALL_2 - np.square(y_WALL_2)/2) - 2.0 * x_WALL_2 / Re
    # p_WALL_2 = 0*(0.0 + x_WALL_2)
    t_WALL_2 = T2*((1.0 - y_WALL_2)/2) 
    WALL_2 = np.concatenate((WALL_2, u_WALL_2, v_WALL_2, p_WALL_2, t_WALL_2), 1)

    x_WALL_3 = WALL_3[:,0:1]
    y_WALL_3 = WALL_3[:,1:2]
    u_WALL_3 = U3*(1.0 + y_WALL_3)
    v_WALL_3 = V3*(1.0 + y_WALL_3)
    p_WALL_3 = Ra / (2.0 * Pr * Re**2) * (y_WALL_3 - np.square(y_WALL_3)/2) - 2.0 * x_WALL_3 / Re
    ...
    p_WALL_4 = Ra / (2.0 * Pr * Re**2) * (y_WALL_4 - np.square(y_WALL_4)/2) - 2.0 * x_WALL_4 / Re
    # p_WALL_4 = 0*(1.0 + y_WALL_4)
    t_WALL_4 = T4*(0.0 + y_WALL_4) 
    ...
    with tf.device('/device:GPU:0'):
        ...
        #Train from scratch
        model = PINN_laminar_flow(XY_c, WALL_1, WALL_2, WALL_3, WALL_4, uvt_layers, lb, ub)

        Niter=10000
        start_time = time.time()
        loss_WALL_1, loss_WALL_2, loss_WALL_3, loss_WALL_4, loss_f, loss = model.train(iter=Niter, learning_rate=5e-3)
        model.train_bfgs()
        print("--- %s seconds ---" % (time.time() - start_time))

        # Save neural network
        model.save_NN('channel2D_Ra3_Dirichlet_p_w12.pickle')

        # Save loss history
        with open('loss_history_channel_Ra3_Dirichlet_p_w12.pickle', 'wb') as f:
            pickle.dump(model.loss_rec, f)


        # # Load trained neural network
        # model = PINN_laminar_flow(XY_c, WALL_1, WALL_2, WALL_3, WALL_4, uvt_layers, lb, ub, ExistModel = 1, uvtDir = 'channel2D_Ra3_Dirichlet_p_w12.pickle')

        
        # Get mixed-form PINN prediction
        x_PINN = np.linspace(0, 2.0, 251)
        y_PINN = np.linspace(-1.0, 1.0, 251)
        ...
        mdic = {"field_MIXED": field_MIXED, "label": "channel2D_Ra3_Dirichlet_p_w12"}
        scipy.io.savemat("channel2D_Ra3_Dirichlet_p_w12.mat",mdict=mdic)

        # Plot the comparison of u, v, p, t
        postProcess(xmin=0, xmax=2.0, ymin=-1, ymax=1.0, field_MIXED=field_MIXED, s=4, alpha=0.5)